...
export class GalleryPhoto {
  ...
  static async setAsCover(id, themeId) {
    ...
    // Get the photo we want to set as cover
    ...
    // Also update the theme's coverImageUrl directly in database
    const db = await getDatabase();
    await db.collection('gallery_themes').updateOne(
      { id: themeId },
      { $set: { coverImageUrl: photo.imageUrl, updatedAt: new Date() } }
    );
