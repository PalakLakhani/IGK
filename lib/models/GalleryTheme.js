...
export class GalleryTheme {
  ...
  static async updatePhotoCount(id, count = null) {
    const collection = await this.getCollection();
    
    // If count not provided, calculate it from photos
    if (count === null) {
      const db = await getDatabase();
      const photoCount = await db.collection('gallery_photos').countDocuments({ themeId: id });
      count = photoCount;
    }
